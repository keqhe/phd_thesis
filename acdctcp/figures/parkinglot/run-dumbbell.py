...
def start_flows(args, run):
	...
	#start clients
	#
	exptime = args.exptime + 1*(len(src) - 1)
	for si in range(0,len(src)):
		print "starting nuttcp on server arldcn%d"%(src[si])
		os.system("ssh arldcn%d nuttcp -t -i 1 -T %d 10.0.2.%d > %s/run%04d/dcn%d-dcn%d &"%(src[si],exptime - si,dst[si], args.dir, run, src[si],dst[si]))	
		if si == (len(src) - 1):
			os.system("ssh arldcn%d /power/home/erozner/sockperf-2.5.241/target/bin/sockperf ping-pong  -p 8899 -i 10.0.2.%d -t %d --tcp --pps=100 --full-log /tmp/sockperf.log &"%(src[si],dst[si], exptime - si))
		time.sleep(1)
	#sleep for a bit

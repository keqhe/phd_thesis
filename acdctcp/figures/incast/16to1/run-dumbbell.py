...
def kill_nuttcp(X):
	...
	os.system("ssh arldcn%d sudo killall -9 iperf"%(X))
def kill_sockperf(X):
...
def start_flows(args, run):
	...
	#start servers
	for d in dst:
		os.system("ssh arldcn%d iperf -s > %s/run%04d/iperf_server &"% (d, args.dir, run))
	...
	for si in range(0,len(src)):
		...
		#os.system("ssh arldcn%d /power/home/keq/workloads/program/acdctcp/customer_sender 10.0.2.%d 5001 %d> %s/run%04d/dcn%d-dcn%d &"%(src[si],dst[0], exptime, args.dir, run, src[si],dst[0]))
		os.system("ssh arldcn%d /power/home/keq/workloads/program/acdctcp/iperf-2.0.5/src/iperf -c 10.0.2.%d -i 1 -t %d -f m > %s/run%04d/dcn%d-dcn%d &"%(src[si],dst[0], exptime, args.dir, run, src[si],dst[0]))
	for si in range(0,1):
		os.system("ssh arldcn%d /power/home/erozner/sockperf-2.5.241/target/bin/sockperf ping-pong  -p 8899 -i 10.0.2.%d -t %d --tcp --pps=100 --full-log /tmp/sockperf.log &"%(src[si],dst[0], args.exptime))
	#sleep for a bit
	time.sleep(args.exptime+5)
	for si in range(0,1):
                cmd = "scp arldcn%d:/tmp/sockperf.log %s/run%04d/sockperf_dcn%d-dcn%d"%(src[si], args.dir, run, src[si],dst[0])
                print cmd
                os.system("scp arldcn%d:/tmp/sockperf.log %s/run%04d/sockperf_dcn%d-dcn%d"%(src[si], args.dir, run, src[si],dst[0]) ); #copy sock log back from each TCP sedner
        time.sleep(1)

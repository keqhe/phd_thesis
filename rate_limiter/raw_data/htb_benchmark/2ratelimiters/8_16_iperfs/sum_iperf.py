...
numIperf = int(sys.argv[1])
rate = int(sys.argv[2]) * 1000
files = [ '%dmbps/%diperfs/limiter1/iperf%dmbps%diperfs%d.log' % (rate, numIperf, rate, numIperf, i) for i in range(1, numIperf+1)] +[ '%dmbps/%diperfs/limiter2/iperf%dmbps%diperfs%d.log' % (rate, numIperf, rate, numIperf, i) for i in range(1, numIperf+1)] 

\section{Design}
\label{rate-limiter:sec:design} \mylabel{sec:design}
\iffalse
...
If we directly apply the existing ECN marking technique onto rate limiter queue, a few 
...
\begin{algorithm}[t]
\caption{Pseudo-code of Algorithm 2}
...
\IF {current\_qlen $\ge$ {\emph {K2}} || current\_qlen > {\emph {K1}} and q.gradient $>$ 0}
\STATE tcp(p).ece $\leftarrow$ 1
\ENDIF
\ENDIF
\ENDFOR
...
Besides the rate limiter queue occupancy, we also consider the trend of queue occupancy change (i.e., the gradient) 
when setting ECE bit in
TCP ACK header. We maintain two thresholds, $K1$ and $K2$ ($K1 < K2$). The queue occupancy gradient is calculated in the
following ways---we first calculate the $new\_gradient$ which is the value of $current\_qlen - prev\_qlen$.
Then $gradient$ is calculated as $$gradient = (1 - \alpha)*gradient + \alpha*new\_gradient$$  

Similar to Algorithm~\ref{alg:algorithm1}, we process each incoming TCP ACK. If the length of the TCP ACK's corresponding
rate limiter queue is larger than or equal to $K2$, 
then we set this ACK's ECE bit to 1. If the length of the TCP ACK's corresponding
rate limiter queue is larger than $K1$ but smaller than $K2$ while the calculated $gradient$ is positive, we set
this ACK's ECE bit to 1. In all other cases, this ACK's ECE bit is left unchanged. The idea is that if the current 
rate limiter queue is larger than $K1$ but it is draining rapidly (i.e., the $gradient$ is negative or 0), we do not
set TCP ECE bit.
To achieve the most accurate estimation of the queue length gradient, we update $gradient$ whenever a packet is processed
by the rate limiter. $gradient$ is a per rate limiter queue parameter.
The details of this algorithm is presented in Algorithm~\ref{alg:algorithm2}.
\fi

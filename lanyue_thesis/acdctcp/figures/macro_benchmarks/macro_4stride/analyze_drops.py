...
for i in xrange(1, 2):
	if i < 10:
		...
		before_file = result + '/' + 'run00' + str(i) + '/' + 'sw-before-' + sw
                after_file = result + '/' + 'run00' + str(i) + '/' + 'sw-after-' + sw

	(stat, out) = commands.getstatusoutput('/power/home/keq/workloads/program/acdctcp/calc_drops.py --before %s --after %s'%(before_file, after_file))
	print stat, out

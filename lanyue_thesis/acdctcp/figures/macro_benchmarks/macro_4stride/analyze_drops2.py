...
for i in xrange(1, 2):
	...
	if i < 10:
		before_file = result + '/' + 'run000' + str(i) + '/' + 'sw-before-' + sw +"-port" + str(j)
		after_file = result + '/' + 'run000' + str(i) + '/' + 'sw-after-' + sw + "-port" + str(j)
	else:
		before_file = result + '/' + 'run00' + str(i) + '/' + 'sw-before-' + sw + "-port" + str(j)
                after_file = result + '/' + 'run00' + str(i) + '/' + 'sw-after-' + sw + "-port" + str(j)

	(stat, out) = commands.getstatusoutput('/power/home/keq/workloads/program/acdctcp/calc_drops2.py --before %s --after %s --port %d'%(before_file, after_file, j))
	print stat, out

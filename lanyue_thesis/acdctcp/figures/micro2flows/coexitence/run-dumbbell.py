...
def start_flows(args, run):
	...
	#start clients
	#CUBIC pair start first, then DCTCP
	flow_duration = 20
	for si in range(0,len(src)):
		print "starting nuttcp on server arldcn%d"%(src[si])
		os.system("ssh arldcn%d nuttcp -t -i 1 -T %d 10.0.2.%d > %s/run%04d/dcn%d-dcn%d &"%(src[si],flow_duration,dst[si], args.dir, run, src[si],dst[si]))
		...
		flow_duration = flow_duration - 10
		time.sleep(5)
	#sleep for a bit
	time.sleep(args.exptime+30)
	for si in range(0,len(src)):
